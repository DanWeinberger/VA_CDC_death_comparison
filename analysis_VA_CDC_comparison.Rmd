...
#multiply these by 4 because we want an annual rate, and we are summing both numerator and denominator over the 4 quarters
b2 <- b1[b1$date>='2020-01-01',] %>%
  group_by(agec, race_recode, sex, region) %>%
  mutate(n_times=n()) %>%
  ungroup() %>%
  group_by(source, agec) %>%
  summarize(sum.obsN= sum(obsN), 
            obs.std.N = 100000*sum( (obsN * std.pop.va/pop)/sum(std.pop.va) )*4 ,
            exp.std.N = 100000*sum( (expectedN * std.pop.va/pop)/sum(std.pop.va)*4 ),
            expN=sum(expectedN), 
            pop=sum(pop) ) %>% #divide pop by 4 bc 4 time point per group
  ungroup() %>%
  mutate('obs_inc'=sum.obsN/pop*100000*4, 'exp_inc'=expN/pop*100000*4, 'ratio'=obs_inc/obs.std.N)
...
b2.m$source <- factor(c2.m$source, c('cdc','va.full','va.users','va.lr'))
...
```{r}
c2 <- b1[b1$date>='2020-01-01',] %>%
  group_by(source) %>%
  summarize(sum.obsN= sum(obsN), 
            obs.std.N = 100000*sum( (obsN * std.pop.va/pop)/sum(std.pop.va) )*4 ,
            exp.std.N = 100000*sum( (expectedN * std.pop.va/pop)/sum(std.pop.va)*4 ),
            expN=sum(expectedN), 
            pop=sum(pop)/4 ) %>% #divide pop by 4 bc 4 time point per group
  ungroup() %>%
  mutate('obs_inc'=sum.obsN/pop*100000, 'exp_inc'=expN/pop*100000, 'excess_inc'=obs_inc-exp_inc, 'excess_inc_std'=obs.std.N - exp.std.N , 'IRR.raw'=obs_inc/exp_inc)
