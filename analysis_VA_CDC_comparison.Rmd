...
ggplot(all.inc, aes(x=agec, y=log(obs_inc),color=source))+
  geom_point() +
  theme_classic()+
  geom_point(data=all.inc, aes(x=agec, y=log(obs_inc-excess_inc_median),color=source), pch=1, position=position_nudge(x=-0.1))+
  
  geom_line(data=all.inc)
  
  ggtitle('Incidence in 2020 (open symbol=expected)')+
  ylab('Log(Incidence)')
...
ggplot(all.inc2.m, aes(x=agec2, y=log(value),color=source, shape=variable))+
  ...
  theme_classic()+
 scale_x_discrete(labels= age.labs1)+
