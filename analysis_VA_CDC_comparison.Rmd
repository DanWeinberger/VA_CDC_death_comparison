...
preds.age.race.region <- bind_rows(cdc$preds.age.race.region, va.users.cr$preds.age.race.region,va.full$preds.age.race.region)
 ...
 p2
 ggsave('rr_region_age.jpg',path = './plots', width = 8, height =6, device='jpg', dpi=400)
...
p.rr <- ggplot(all.std.rrs.plot, aes(x=date , y=value, group=label, col=label)) +
         geom_line() +
         theme_classic() +
  facet_wrap(~variable) +
...
ggsave('rr_plot.jpg',path = './plots', width = 6, height = 3, device='jpg', dpi=400)
...
b2.sex <- b1.sex[b1.sex$date>='2020-04-01',] %>%
...
b2.race_recode <- b1.race_recode[b1.race_recode$date>='2020-04-01',] %>%
