...
std.pop.va <- va.full$preds.age.race.region[va.full$preds.age.race.region$date=='2020-01-01', c('agec','sex','race_recode','region', 'pop')]
names(std.pop.va) <- c('agec','sex','race_recode','region', 'std.pop.va')

a1 <- list(cdc$preds.age.race.region, va.full$preds.age.race.region , va.users.cr$preds.age.race.region)
names(a1) <- c('US','VA enrollees','VA active users')
...
names(a2) <-  c('US','VA enrollees','VA active users')
...
std_rr_func <- function( y , M_only=F){
    ...
    rr.ave <- a2 %>%
      group_by(date) %>%
      summarize('sum_obs_std'= sum((obsN+0.5)*std.pop.va/pop), 
                'sum_expected_std'= sum((expectedN+0.5)*std.pop.va/pop),
                'sum_obs'= sum((obsN+0.5)), 
                'sum_expected'= sum((expectedN+0.5))
...
ggplot(comp.rr1[comp.rr1$obsN.y>=0 ,], aes(x=RR_median.x, y=RR_median.y, group=region, col=region, size=obsN.y))+
  geom_point()+
  geom_abline()+
  theme_classic()+
...
ggplot(comp.rr1[comp.rr1$obsN.y>=0 ,], aes(x=obs_inc.x, y=obs_inc.y, group=region, col=region))+
  geom_point()+
  geom_abline()+
  theme_classic()+
  ylim(0, 25000)+
...
b2 <- b1[b1$date>='2020-01-01',] %>%
  ...
  summarize(obsN= sum(obsN), expN=sum(expectedN), pop=sum(pop)/4) %>% #divide pop by 4 bc 4 time point per groi[]
  ungroup() %>%
  mutate('obs_inc'=obsN/pop*100000, 'exp_inc'=expN/pop*100000)
