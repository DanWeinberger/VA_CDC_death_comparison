...
q2 <- grid.arrange(t3.cdc, t3.va.full)
...
# ggsave('Fig S1 rr_region_age.jpg',path = './plots', width = 8, height =6, device='jpg', dpi=400)
 ...
 p.rr_region <- ggplot(plot.ds, aes(x=qdate , y=RR_std_median, group=labels, col=labels ,linetype=labels)) +
         geom_line(alpha=0.5) +
          geom_point(data=plot.ds ,aes(x=qdate , y=RR_std_median,group=labels, shape=labels))+
         theme_classic() +
  facet_grid(region~agec, scales='fixed') +
...
p.rr <- ggplot(all.std.rrs.plot, aes(x=qdate , y=value, group=label, col=label ,linetype=label)) +
         geom_line(alpha=0.5) +
          geom_point(data=all.std.rrs.plot ,aes(x=qdate , y=value,group=label, shape=label))+
         theme_classic() +
...
c2.m.age$agec2[c2.m.age$variable=='exp.std.N'] <- c2.m.age$agec2[c2.m.age$variable=='exp.std.N'] - 0.1
...
c2.m.sex$sex2[c2.m.sex$variable=='exp.std.N'] <- c2.m.sex$sex2[c2.m.sex$variable=='exp.std.N'] - 0.1

c2.m.sex$agec2[c2.m.sex$variable=='exp.std.N'] <- c2.m.sex$agec2[c2.m.sex$variable=='exp.std.N'] - 0.1
...
c2.m.race_recode$race_recode2[c2.m.race_recode$variable=='exp.std.N'] <- c2.m.race_recode$race_recode2[c2.m.race_recode$variable=='exp.std.N'] - 0.1

c2.m.race_recode$agec2[c2.m.race_recode$variable=='exp.std.N'] <- c2.m.race_recode$agec2[c2.m.race_recode$variable=='exp.std.N'] - 0.1
