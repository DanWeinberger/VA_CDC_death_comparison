...
dic.cdc <- cbind.data.frame('test.tau'=test.tau, 'dic'=sapply(mod.comp.cdc, '[[','dic'))

plot(dic.va$test.tau, dic.va$dic-min(dic.va$dic), type='l')
points(dic.cdc$test.tau, dic.cdc$dic-min(dic.cdc$dic), type='l', col='red')
abline(h=2)
...
mod1 <- pblapply(combine.data, inla_model1,prec.prior1=0.5)
...
```{r}
 obs_exp_plot <- function(ds, xvar, yvar, groupvar, colvar, datemin='2014-01-01'){
   ds %>%
...
p1 <- obs_exp_plot(ds=preds.age[preds.age$source=='cdc',], xvar='date',yvar='N_deaths', groupvar='agec', colvar='agec')
...
p1 <- obs_exp_plot(ds=preds.age_race[preds.age_race$source=='va',], xvar='date',yvar='N_deaths', groupvar='agec', colvar='agec', datemin='2018-01-01')
...
Age and region 
```{r, fig.width=8, fig.height=6}
p1 <- obs_exp_plot(ds=preds.age.region[preds.age.region$source=='cdc',], xvar='date',yvar='N_deaths', groupvar='agec', colvar='agec', datemin='2018-01-01')
