...
p0 <- ggplot(comp_inc,aes(x=inc_cdc, y=inc_va)) +
  geom_point() +
  theme_classic() +
      facet_wrap(~race_recode,  nrow=1,scales='free' ) +
...
mod1 <- pblapply(combine.data, inla_model3)
...
p1 <- RR_plot(ds=preds.age.race.region[ preds.age.race.region$race_recode=='1',] )
p1 + facet_grid(region~agec, scales='free')
...
```{r, fig.width=8, fig.height=6}
p1 <- RR_plot(ds=preds.age.region[preds.age.region$agec !='Under 25 Years',])
p1 + facet_grid(region~agec, scales='free')
